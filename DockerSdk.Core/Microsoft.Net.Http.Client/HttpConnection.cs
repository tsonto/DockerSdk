    ...
    internal class HttpConnection : IDisposable
    {
        ...
        public async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken ct)
        {
            try
            {
                ...
                await Transport.WriteAsync(requestBytes, 0, requestBytes.Length, ct);

                if (request.Content != null)
                {
                    if (request.Content.Headers.ContentLength.HasValue)
                    {
                        await request.Content.CopyToAsync(Transport, ct);
                    }
                    else
                    {
                        ...
                        {
                            await request.Content.CopyToAsync(chunkedStream, ct);
                            await chunkedStream.EndContentAsync(ct);
                        }
                    }
                }

                // Receive headers
                List<string> responseLines = await ReadResponseLinesAsync(ct);
